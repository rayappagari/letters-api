...
def lambda_handler(event: APIGatewayProxyEvent, context: LambdaContext) -> Dict[str, Any]:
    ...
    return create_letter_in_s3(dynamo_db = dynamodb_resource_class,
...
def create_letter_in_s3(dynamo_db: LambdaDynamoDBClass,
    ...
    try:
        response = dynamo_db.table.get_item(Key={"cust_id": f"{cust_id}"})["Item"]
        #document_text = dynamo_db.table.get_item(Key={"doc_type": f"{doc_type}"})["Item"]

        customer_name = response["cust_id"]
        document_text = response["doc_type"]
